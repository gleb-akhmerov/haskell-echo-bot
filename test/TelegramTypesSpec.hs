...
spec = do
  describe "parser" $ do
    it "understands result, updates and text messages" $ do
      ...
      res `shouldBe` Just (Result {rOk = True, rDescription = Nothing, rResult = Just [Update {uUpdateId = 39473849, uMessage = Just (Message {mFrom = User {uId = 123456789}, mText = Just "/help"}), uCallbackQuery = Nothing}]})
