...
spec = do
  describe "Bot.react" $ do
    ...
    it "echoes any non-command message n times, n is configurable" $ do
      runNoLogging (evalBotT (react config (InTextMessage "text" Mes)) 1)
        `shouldBe` EchoTimes 1 Mes
      runNoLogging (evalBotT (react config (InTextMessage "text" Mes)) 2)
        `shouldBe` EchoTimes 2 Mes

    it "replies with the help message when it receives /help command" $ do
      runNoLogging (evalBotT (react config (InTextMessage "/help" Mes)) 1)
        `shouldBe` SendText (helpText config)

    it "sends the repeat keyboard when it receives /repeat command" $ do
      runNoLogging (evalBotT (react config (InTextMessage "/repeat" Mes)) 1)
    ...
    it "sets the number of repeats when the user pushes a button on the keyboard" $ do
      runNoLogging (runBotT (react config ((KeyboardKeyPushed 5) :: InMessage ())) 1)
      ...
      runNoLogging (evalBotT conversation 1) `shouldBe` (EchoTimes 5 Mes)
